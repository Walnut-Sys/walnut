...
export const fillSVGTemplate = jest.fn(
  async (parserOutput: IParserOutput): Promise<Buffer> => {
    const pathToTemplate = path.resolve(__dirname, './svg-template.svg');
    const svgTemplate = (await fs.promises.readFile(pathToTemplate)).toString();
